...
def insert():
    ...
    try:
        with app_server.app_context():
            ...
            if request.json.get("steps"):
                # json.dumps将对象转换成json格式的字符串
                request.json["steps"] = json.dumps(request.json["steps"])
                print("steps", request.json["steps"])
...
def process_with_ai_stream():
    ...
    try:
        ...
        with app_server.app_context():
            ...
            # 查询需求内容
            primary_doc = ApiDocument.query.filter_by(id=int(primary_content)).first()
            if not primary_doc:
            ...
            if second_content is not None and len(second_content) > 0:
                for id in second_content.split(','):
                    secondary_doc = ApiDocument.query.filter_by(id=int(id)).first()
                    if secondary_doc:
        ...
        # 加载需求评审提示词
        ai_prompt = load_prompt("../prompts", "提示词-接口测试用例生成.txt", {
