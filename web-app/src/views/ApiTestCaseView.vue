<template>
    ...
    <el-dialog v-model="addOrEditModalVisible" :title="addOrEditModalTitle" width="800px">
        <el-form :model="addOrEditForm" label-width="100px" ref="addOrEditFormRef">
            ...
            <el-divider content-position="center">接口请求参数</el-divider>
            <el-form-item label="接口路径" prop="steps['path']" required>
                <el-input v-model="addOrEditForm.steps['path']" :rows="2" placeholder="请输入接口路径" />
            </el-form-item>
            <el-form-item label="请求方式" prop="steps['method']" required>
                <el-input v-model="addOrEditForm.steps['method']" :rows="2" placeholder="请输入请求方式" />
            </el-form-item>
            <el-form-item label="URL参数" prop="steps['params']">
                <el-input v-model="addOrEditForm.steps['params']" type="textarea" :rows="2" placeholder="请输入URL参数" />
            </el-form-item>
            <el-form-item label="表单数据" prop="steps['data']">
                <el-input v-model="addOrEditForm.steps['data']" type="textarea" :rows="2" placeholder="请输入FORM表单数据" />
            </el-form-item>
            <el-form-item label="JSON数据" prop="steps['json']">
                <el-input v-model="addOrEditForm.steps['json']" type="textarea" :rows="2" placeholder="请输入JSON数据" />
            </el-form-item>
            <el-form-item label="Cookies" prop="steps['cookies']">
                <el-input v-model="addOrEditForm.steps['cookies']" type="textarea" :rows="2" placeholder="请输入Cookies" />
            </el-form-item>
            <el-form-item label="Headers" prop="steps['headers']">
                <el-input v-model="addOrEditForm.steps['headers']" type="textarea" :rows="2" placeholder="请输入Headers" />
            </el-form-item>


            <el-form-item label="预期结果" prop="expected" required>
                <el-input v-model="addOrEditForm.expected" type="textarea" :rows="5" placeholder="请输入预期结果" />
            </el-form-item>
        </el-form>
        <template #footer>
...
import api from '@/api/api_test_case_api'
...
const handleAdd = () => {
    ...
    addOrEditForm.name = ''
    addOrEditForm.module_id = curModuleId.value
    addOrEditForm.priority = '0'
    addOrEditForm.precondition = ''
    addOrEditForm.steps = {}
    addOrEditForm.expected = ''
    // 显示模态框
...
async function handleEdit(item) {
    ...
    if (resData.code == 200) {
        // 针对reactive类型不支持以下方式来修改数据
        // addOrEditForm = resData.data
        Object.assign(addOrEditForm, resData.data)
        // 将steps中的内容从json格式的字符串转换成json对象
        addOrEditForm.steps = JSON.parse(addOrEditForm.steps)
        // addOrEditForm.steps.params = JSON.stringify(addOrEditForm.steps.params)
        // 批量转换
        Object.keys(addOrEditForm.steps).forEach(field => {
            // 批量处理对象属性时，可以使用中括号语法来操作属性
            let val = addOrEditForm.steps[field]
            if (typeof val == 'object') {
                addOrEditForm.steps[field] = JSON.stringify(val)
            }
        })
    } else {
...
async function handleSave() {
    if (!addOrEditForm.name) {
        ElMessage.error('请输入用例名称')
        return
    }
    if (!addOrEditForm.priority) {
        ElMessage.error('请选择优先级')
        return
    }
    if (!addOrEditForm.steps) {
        ElMessage.error('请输入测试步骤')
        return
    }
    if (!addOrEditForm.expected) {
    ...
    try {
        let params = {
            project_id: project_id,
            name: addOrEditForm.name,
            module_id: addOrEditForm.module_id,
            priority: addOrEditForm.priority,
            precondition: addOrEditForm.precondition,
            steps: addOrEditForm.steps,
            expected: addOrEditForm.expected,
        }
        let res
        if (addOrEditForm.id) {
            // 编辑保存
            params.id = addOrEditForm.id
...
function handleNodeClick(node, data) {
    ...
    // 添加用例需要指定模块ID
    addOrEditForm.module_id = node.source.id
