<template>
        ...
        <el-form label-width="80px" @submit.prevent="confirmMove">
            ...
            <el-form-item label="目标父目录">
                <el-tree-select
                    ...
                    node-key="id"
                    label="name"
                    placeholder="根目录（清空选择或直接不选）"
                    :render-after-expand="false"
                    clearable
                    style="width: 100%;"
                />
                <div class="el-form-item__tip">清空选择表示移动到根目录</div>
            </el-form-item>
        </el-form>
    ...
    <el-dialog v-model="aiModalVisible" :title="'AI 评审 - ' + addOrEditForm.name" width="800px" draggable :modal="false"
        ...
        <template #footer>
            ...
            <el-button type="success" @click="applySuggest">应用</el-button>
...
import api from '@/api/api_document_api'
...
async function saveTree() {
    ...
    try {
        ...
        if (addOrEditForm.value.id) {
            ...
            // 添加保存
            // 添加保存 - 只传project_id、parent_id、name，不传id
            const params = {
                project_id: project_id,
...
/**
 * 处理节点下拉菜单命令
 * @param command 命令类型：add/edit/sort/move/delete
 * @param node 节点对象
 * @param source 节点源数据
 */
