...
const handleView = async (row) => {
    try {
        ...
        if (resData.code == 200) {
            Object.assign(viewData.value, resData.data)
            if (typeof viewData.value.details === 'string') {
                viewData.value.details = JSON.parse(viewData.value.details)

                if (typeof viewData.value.details.steps === 'string') {
                    viewData.value.details.steps = JSON.parse(viewData.value.details.steps)
                }
            }
...
.el-card :deep(.el-table__body-wrapper)::-webkit-scrollbar {
    width: 6px;
    height: 6px;
}

.el-card :deep(.el-table__body-wrapper)::-webkit-scrollbar-track {
    background: #f1f3f4;
    border-radius: 3px;
}

.el-card :deep(.el-table__body-wrapper)::-webkit-scrollbar-thumb {
    background: #c1c6cd;
    border-radius: 3px;
    transition: all 0.2s;
}

.el-card :deep(.el-table__body-wrapper)::-webkit-scrollbar-thumb:hover {
    background: #909399;
}
