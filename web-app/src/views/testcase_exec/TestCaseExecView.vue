                        ...
                        <div class="card-header">
                            ...
                            <div class="action-buttons">
                                <el-select v-model="tmp_exec_type" placeholder="选择用例类型">
                                    <el-option label="WEB 网页" value="web" selected />
                                    <el-option label="安卓设备" value="android" />
...
import test_case_api from '@/api/test_case_api'
...
async function loadTestCases() {
    loading.value = true
    // 加载测试用例数据
    let res = await test_case_api.queryByPage(queryParams)
...
const handleView = async (row) => {
    try {
        // 获取完整的测试用例数据
        const res = await test_case_api.queryById(row.id)
